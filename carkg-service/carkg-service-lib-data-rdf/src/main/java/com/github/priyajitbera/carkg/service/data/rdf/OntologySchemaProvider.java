...
public class OntologySchemaProvider {
  ...
  public OntologySchemaProvider(
    ...
    Arrays.stream(OntologySchemaExtractor.KNOWN_FORMATS)
        .forEach(
            format -> {
              String fileName =
                  OntologySchemaExtractor.rdfFormatToConventionalOntologySchemaFileName(format);
              String filePath = ontologySchemaDir + "/" + fileName;
              Resource resource = new ClassPathResource(filePath);
              if (resource.exists()) {
                try {
                  this.formatToOntology.put(
                      format, resource.getContentAsString(StandardCharsets.UTF_8));
                } catch (IOException e) {
