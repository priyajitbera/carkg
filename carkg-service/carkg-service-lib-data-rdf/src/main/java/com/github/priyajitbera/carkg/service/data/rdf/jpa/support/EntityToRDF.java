...
public class EntityToRDF {
  ...
  private final Set<Class<?>> wrapperTypes =
      Set.of(Integer.class, Long.class, Float.class, Double.class, String.class);

  private final String NAMESPACE_ONTOLOGY;
  private final String NAMESPACE_DATA;

  public EntityToRDF(
      @Value("${namespace.ontology}") String namespaceOntology,
      @Value("${namespace.data}") String namespaceData) {
    this.NAMESPACE_ONTOLOGY = namespaceOntology;
    this.NAMESPACE_DATA = namespaceData;
  }

  public <T extends Identifiable> Model entityToOntologyModel(
      Class<T> entityClass, Class<?> jsonView) {
    Model model = ModelFactory.createDefaultModel();
    entityToOntologyResource(model, entityClass, jsonView);
    return model;
  }

  private Map.Entry<Resource, Property> entityToOntologyResource(
      Model model, Class<?> entityClass, Class<?> viewAnnotation) {
    ...
    for (Field field : entityClass.getDeclaredFields()) {
      ...
      Property property =
          model.createProperty(NAMESPACE_ONTOLOGY, field.getAnnotation(RdfPredicate.class).value());

      if (!field.isAnnotationPresent(JsonView.class)
          || Arrays.stream(field.getAnnotation(JsonView.class).value())
              .sequential()
              .noneMatch(jsonView -> jsonView == viewAnnotation)) {
        continue;
      }
      if (field.getType().isAnnotationPresent(Entity.class)) {
        ...
        property.addProperty(
            RDFS.domain, resourceType); // example: The subject of hasBrand must be a Car
        Map.Entry<Resource, Property> propertyResource =
            entityToOntologyResource(model, field.getType(), viewAnnotation);
        property.addProperty(
            RDFS.range,
            propertyResource.getValue()); // example: The object of hasBrand must be a Brand
        ...
        if (genericType instanceof ParameterizedType) {
          ...
          if (genericTypeClazz.isAnnotationPresent(Entity.class)) {
            ...
            property.addProperty(
                RDFS.domain, resourceType); // example: The subject of hasBrand must be a Car
            Map.Entry<Resource, Property> propertyResource =
                entityToOntologyResource(model, genericTypeClazz, viewAnnotation);
            property.addProperty(
                RDFS.range,
                propertyResource.getValue()); // example: The object of hasBrand must be a Brand
            property.addProperty(RDFS.label, field.getAnnotation(RdfPredicate.class).label());
  ...
  private <T extends Identifiable> Map.Entry<Resource, Property> entityToDataModel(
      Model model, T entity, Class<?> viewAnnotation) {
    String uri = NAMESPACE_DATA + entity.getId();
    Resource resource = model.createResource(uri);

    Property resourceType =
        model.createProperty(NAMESPACE_ONTOLOGY + entity.getClass().getSimpleName());
    ...
    for (Field field : entity.getClass().getDeclaredFields()) {
      ...
      if (fieldValue != null) {
        if (field.getType().isPrimitive() || wrapperTypes.contains(field.getType())) {
          ...
          Map.Entry<Resource, Property> propertyResource =
              entityToDataModel(model, (Identifiable) fieldValue, viewAnnotation);
          ...
          if (genericType instanceof ParameterizedType) {
            ...
            if (genericTypeClazz.isAnnotationPresent(Entity.class)) {
              ...
              collection.forEach(
                  item -> {
                    Map.Entry<Resource, Property> propertyResource =
                        entityToDataModel(model, (Identifiable) item, viewAnnotation);
                    resource.addProperty(property, propertyResource.getKey());
                  });
            }
          }
