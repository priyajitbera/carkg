...
public abstract class AbstractWorkflow implements Workflow {
  ...
  public AbstractWorkflow(
      GenerationClient generationClient, SyncMcpToolCallbackProvider toolCallbackProvider) {
  ...
  public void run(String taskPrompt) {
    ...
    while (!stepsToExecute.isEmpty()) {
      ...
      chat.getChatItems()
          .add(
              ChatItem.builder()
                  ...
                  .clientToModel(ClientToModelChatItem.builder().prompt(stepPrompt).build())
                  .build());
      ...
      List<com.github.priyajitbera.carkg.service.model.client.dto.request.Tool> tools =
          mapTools(getTools());
      ...
      chat.getChatItems()
          .add(
              ChatItem.builder()
                  ...
                  .modelToClient(
                      ModelToClientChatItem.builder()
                          .response(Serializer.toTokenOriented(generationResponse))
                          .build())
                  .build());

      Response.InstructionType instructionType =
          (generationResponse.getContent() == null
                      && generationResponse.getToolCallDetails() == null)
                  || (generationResponse.getContent() != null
                      && generationResponse.getContent().contains("END_TASK"))
      ...
      final Response response =
          Response.builder()
              ...
              .messageDetails(
                  MessageDetails.builder().message(generationResponse.getContent()).build())
              .toolCallDetails(
                  generationResponse.getToolCallDetails() == null
                      ? null
                      : mapToolCallDetails(generationResponse.getToolCallDetails()))
              .build();
  ...
  List<com.github.priyajitbera.carkg.service.model.client.dto.request.Tool> mapTools(
      List<Tool> tools) {
    return tools.stream()
        .map(
            tool ->
                com.github.priyajitbera.carkg.service.model.client.dto.request.Tool.builder()
                    .name(tool.getName())
  ...
  ToolCallDetails mapToolCallDetails(
      com.github.priyajitbera.carkg.service.model.client.dto.response.ToolCallDetails
          toolCallDetails) {
  ...
  void executeToolCall(ToolCallDetails toolCallDetails, Chat chat) {
    ...
    var toolCallback =
        Arrays.stream(toolCallbackProvider.getToolCallbacks())
            .filter(
                callback -> callback.getToolDefinition().name().equals(toolCallDetails.getName()))
            .findFirst()
            .orElseThrow(
                () ->
                    new RuntimeException("No tool found with name: " + toolCallDetails.getName()));
    ...
    try {
      ...
      log.info(
          "After executing Tool: {} toolCallOutputText: {}",
          toolCallDetails.getName(),
          toolCallOutputText);
      ...
      try {
        chatItem =
            ChatItem.builder()
                ...
                .toolCall(
                    ToolCallChatItem.<JsonNode, String>builder()
                        .input(objectMapper.readTree(toolInput))
                        .output(errorMessage)
                        .build())
                .build();
      ...
      chat.getChatItems().add(chatItem);
      return;
    }
    ...
    try {
      chatItem =
          ChatItem.builder()
              ...
              .toolCall(
                  ToolCallChatItem.<JsonNode, JsonNode>builder()
                      .input(objectMapper.readTree(toolInput))
                      .output(objectMapper.readTree((toolResponse.getText())))
                      .build())
              .build();
