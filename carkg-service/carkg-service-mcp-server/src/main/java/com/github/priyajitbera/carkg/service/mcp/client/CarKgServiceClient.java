...
public class CarKgServiceClient {
  ...
  public CarModel findCarById(String id) {
    return webClient.get().uri("/car/" + id).retrieve().bodyToMono(CarModel.class).block();
  }
  ...
  public List<CarSemanticSearchModel> semanticSearchCar(@RequestParam("query") String query) {
    return webClient
        ...
        .uri(
            uriBuilder ->
                uriBuilder.path("/car/semantic-search").queryParam("query", query).build())
        .retrieve()
  ...
  public BrandModel findBrandById(String id) {
    return webClient.get().uri("/brand/" + id).retrieve().bodyToMono(BrandModel.class).block();
  }
  ...
  public List<BrandSemanticSearchModel> semanticSearchBrand(@RequestParam("query") String query) {
    return webClient
        ...
        .uri(
            uriBuilder ->
                uriBuilder.path("/brand/semantic-search").queryParam("query", query).build())
        .retrieve()
