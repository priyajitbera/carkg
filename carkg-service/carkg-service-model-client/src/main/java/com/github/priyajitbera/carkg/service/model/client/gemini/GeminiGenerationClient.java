...
public class GeminiGenerationClient implements GenerationClient {
  ...
  public GeminiGenerationClient(
      @Value("${gemini.baseUrl}") String geminiBaseUrl,
      @Value("${gemini.apiKey}") String geminiApiKey,
      @Value("${gemini.generative.modelUri}") String geminiGenerativeModelUri) {
  ...
  public String generate(String prompt) {
    GenerationRequest generationRequest =
        GenerationRequest.builder()
            .contents(
                List.of(
                    Content.builder().parts(List.of(Part.builder().text(prompt).build())).build()))
            .build();
    GenerationResponse generationResponse = generate(generationRequest);
    return generationResponse.getCandidates().get(0).getContent().getParts().get(0).getText();
  }

  @Override
  public com.github.priyajitbera.carkg.service.model.client.dto.response.GenerationResponse
      generate(
          String prompt,
    ...
    GenerationRequest generationRequest =
        GenerationRequest.builder()
            ...
            .tools(
                List.of(
                    Tool.builder()
                        .functionDeclarations(tools.stream().map(this::map).toList())
                        .build()))
            .build();
    GenerationResponse generationResponse = generate(generationRequest);
        ...
        .forEach(
            candidate -> {
              candidate
                  ...
                  .forEach(
                      part -> {
                        if (part.getText() != null) contentBuilder.append(part.getText());
                      });
            });
    final String content = contentBuilder.toString();

    Optional<FunctionCall> functionCallOpt =
        generationResponse.getCandidates().get(0).getContent().getParts().stream()
            .filter(part -> part.getFunctionCall() != null)
            .findFirst()
            .map(
                com.github.priyajitbera.carkg.service.model.client.gemini.response.Part
                    ::getFunctionCall);

    return com.github.priyajitbera.carkg.service.model.client.dto.response.GenerationResponse
        .builder()
        .content(content)
        .toolCallDetails(
            functionCallOpt
                .map(
                    functionCall ->
                        com.github.priyajitbera.carkg.service.model.client.dto.response
  ...
  private FunctionDeclaration map(
      com.github.priyajitbera.carkg.service.model.client.dto.request.Tool tool) {
  ...
  private GenerationResponse generate(GenerationRequest generationRequest) {
    ...
    try {
      ...
      log.error(
          "Error while calling Gemini Generative Model API: Status Code: {}, Response Body: {}",
          e.getStatusCode(),
