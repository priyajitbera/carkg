...
public interface Serializer {
  ...
  /**
   * Serializes the object to a token-oriented format using Jackson YAML. This serves as an
   * alternative implementation using a popular library.
   */
  ...
  class ToonConverter {
    ...
    private static void convert(
        com.fasterxml.jackson.databind.JsonNode node, int level, StringBuilder sb, boolean isRoot) {
      if (node.isObject()) {
        if (!isRoot) sb.append("\n");
        ...
        while (fields.hasNext()) {
          ...
          if (fields.hasNext()
              && !value.isObject()
              && !(value.isArray() && !isPrimitiveArray(value))) {
            sb.append("\n");
          }
        }
        ...
        if (isPrimitiveArray(node)) {
          ...
          for (com.fasterxml.jackson.databind.JsonNode item : node) {
            if (item.isObject()) {
              ...
              while (fieldNames.hasNext()) keys.add(fieldNames.next());
            }
          }
