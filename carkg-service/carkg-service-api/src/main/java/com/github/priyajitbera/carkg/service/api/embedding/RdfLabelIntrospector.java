...
public class RdfLabelIntrospector extends JacksonAnnotationIntrospector {
  ...
  private final Map<Annotated, PropertyName> cacheFindNameForSerialization =
      new ConcurrentHashMap<>();
  private final Map<Annotated, PropertyName> cacheFindNameForDeserialization =
      new ConcurrentHashMap<>();

  @Override
  public PropertyName findNameForSerialization(Annotated a) {
    return cacheFindNameForSerialization.computeIfAbsent(
        a,
        key -> {
  ...
  public PropertyName findNameForDeserialization(Annotated a) {
    return cacheFindNameForDeserialization.computeIfAbsent(
        a,
        key -> {
          RdfPredicate rdfPredicate = a.getAnnotation(RdfPredicate.class);
          if (rdfPredicate != null && !rdfPredicate.label().isEmpty()) {
